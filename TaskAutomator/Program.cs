...
while (true) // Keep listening to user input
{
    ...
    switch (choice)
    {
        case "1":
            ...
            Console.Write("Enter the minimum file size in MB (optional): ");
            int minSize = int.TryParse(Console.ReadLine(), out minSize) ? minSize : default; // Added basic parsing with fallback

            Console.Write("Enter the maximum file size in MB (optional): ");
            int maxSize = int.TryParse(Console.ReadLine(), out maxSize) ? maxSize : default; // Added basic parsing with fallback

            action = () => BulkActions.Media.MoveFilesIntoTargetDirectory(sourceDirectory, targetDirectory, fileExtensions, minSize, maxSize);
            break;
        case "2":
            Console.Write("Enter the target directory name: ");
